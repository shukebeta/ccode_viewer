...
describe('mapSessionMessages', () => {
  ...
  it('annotates local-command wrappers with isNoiseWrapper without filtering tool_result text', async () => {
    const lines = [
      {
        type: 'user',
        isMeta: true,
        ...
        message: { role: 'user', content: '<local-command-caveat>The messages below were generated by the user.</local-command-caveat>' }
      },
      {
        type: 'user',
        uuid: 'u1',
        ...
        message: { role: 'user', content: 'hello, run a search' }
      },
      {
        type: 'assistant',
        uuid: 'a1',
        ...
        message: {
          role: 'assistant',
          content: [
            { type: 'tool_use', id: 'tu1', name: 'Bash', input: { command: 'grep foo' } },
            { type: 'tool_result', tool_use_id: 'tu1', content: '<local-command-stdout>matched-line</local-command-stdout>' }
          ]
        }
      },
      {
        ...
        uuid: 'stdout1',
        userType: 'external',
        timestamp: '2025-09-20T00:00:03.000Z',
        message: { role: 'user', content: '<local-command-stdout>captured output</local-command-stdout>' }
      }
    ]
    const file = writeTempJsonl(lines)
    const out = await mapSessionMessages(file)

    const normalUser = out.users.find(u => u.id === 'u1')
    expect(normalUser).toBeDefined()
    expect(normalUser.isNoiseWrapper).toBeUndefined()

    const stdoutUser = out.users.find(u => u.id === 'stdout1')
    expect(stdoutUser).toBeDefined()
    expect(stdoutUser.isNoiseWrapper).toBe(true)
    expect(stdoutUser.noiseKind).toBe('local_command')
    expect(stdoutUser.noiseSubtype).toBe('stdout')

    const allMapEntries = Object.values(out.mapping).flat()
    const assistantEntry = allMapEntries.find(a => a.id === 'a1')
    expect(assistantEntry).toBeDefined()
    expect(assistantEntry.isNoiseWrapper).toBeUndefined()

    const caveatEntry = allMapEntries.find(a => a.id === 'caveat1')
    expect(caveatEntry).toBeDefined()
    expect(caveatEntry.isNoiseWrapper).toBe(true)
    expect(caveatEntry.noiseSubtype).toBe('caveat')
  })
